...
function startGame() {
    ...
    pScore=document.getElementsByClassName("scores")[0];
    pScore.innerHTML="Ваше количество очков: "+score;
    snake_timer = setInterval(move, SNAKE_SPEED);//каждые 200мс запускаем функцию move
    setTimeout(createFood, 5000);
    setTimeout(createMine, 6000);
    kolMine++;
}
...
function haveFoodAndMine(unit) {
    ...
    if(unit_classes.includes('mines-unit')){
        finishTheGame();
    }
    return check;
}
...
function createMine() { ///////////////////////////////////////////////////////////////////////
    var createMine = false;

    while (!createMine) { //пока мину не создали
        // рандом
        kolMine++;
        ...
        if(kolMine%2==0||kolMine%4==0){
            var mine_old = document.getElementsByClassName('cell-' + masMine[masMine.length-1] + '-' + masMine[masMine.length-2])[0];
           mine_old.classList.remove("mines-unit");
           masMine.pop();
           masMine.pop();
            masMine.push(mine_x);
            masMine.push(mine_y);
        ...
        if (!mine_cell_classes.includes('snake-unit')) {
            ...
            mine_cell.setAttribute('class', classes + 'mines-unit');
            createMine = true;
        }
    }
}
